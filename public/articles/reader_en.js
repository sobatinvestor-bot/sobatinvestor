/* Sobat Investor — Reader mode (book-style pagination), ENGLISH strings.
   Identical logic to reader.js (Indonesian) — a separate file so the
   Indonesian reader pages already live in production are never touched.
   + draggable Table-of-Contents bubble (iPhone AssistiveTouch style).
   External for CSP-safety. No library. */
(function () {
  ...
  if (btnMark) btnMark.addEventListener('click', function () {
    var bm = getInt(KEY + ':mark', -1);
    if (bm === cur) { localStorage.removeItem(KEY + ':mark'); toastMsg('Bookmark removed'); }
    else { setInt(KEY + ':mark', cur); toastMsg('Marked page ' + (cur + 1)); }
    render();
  });
  ...
  function buildElements() {
    ...
    bubble.setAttribute('aria-label', 'Table of contents');
    ...
    head.innerHTML = '<span>Contents</span><button class="x" aria-label="Close">&times;</button>';
  ...
  function buildTOC() {
    ...
    if (!hs.length) { list.innerHTML = '<div class="toc-empty">No chapters.</div>'; return; }
    var items = [];
    hs.forEach(function (h) {
      ...
      var p = document.createElement('span'); p.className = 'toc-p'; p.textContent = 'p. ' + (pg + 1);
  ...
  function init() {
    ...
    setTimeout(function () {
      ...
      if (last > 0 && last < pages) { goto(last, false); toastMsg('Resuming from page ' + (last + 1)); }
